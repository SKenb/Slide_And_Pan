...
void handleTimelapse() {
    //server.sendHeader("Location", "",true);   //Redirect to our html web page  
    String site = "<!doctype html><html lang='en'>            <head>            <meta charset='utf-8'>            <meta charset='utf-8'>            <meta name='viewport' content='width=device-width, initial-scale=1.0'>            <title>Slide and Pan</title>            <meta name='description' content='Slide and Pan'>            <meta name='author' content='Sebastian Knoll'>            </head>          <body>              <style>            * {              font-family:courier,arial,helvetica;            }            body {              background-color: rgb(180, 180, 180);            }            li {              display:inline-block;              margin: 5px;            }            ul {              margin: 0 auto;              display: inherit;            }            h1, h2, h3 {              text-align: center;            }            #container {              width: 100%;              max-width: 960px;              margin: 0 auto;            }            .section {              background: whitesmoke;              border-radius: 20px;              padding: 20px;            }            input {              width: 50px;            }            .set_button {              width: 200px;            }            .block {              display: block;              width: 100%;              height: 50px;;            }          </style>                      <script>            function makeRequest(command) {              var url = '/device/command/?' + command;                            var request = new XMLHttpRequest();              request.onreadystatechange = function() {                if (this.readyState == 4 && this.status == 200) {                  location.reload();                }              };              request.open('GET',url);              request.send();            }            function getDaytimeInSeconds() {              let now = new Date();              return now.getSeconds() + 60 * now.getMinutes() + 60*60* now.getHours();             }            var updateTimeRequest = new XMLHttpRequest();            updateTimeRequest.open('GET', '/device/command/?set_time=' + getDaytimeInSeconds());            updateTimeRequest.send();          </script>            <div id='container'>    <h1>Timelapse</h1>    <ul>      <li><a href='index'>Control</a></li>      <li><a href='timelapse'>Timelapse</a></li>      <li><a href='settings'>Settings</a></li>    </ul>    <br>    <div class='section'>      <h2>Milkyway</h2>      <br>      <p style='display: inline-block;'>Slide:</p>       <input value='" + String(getTimelapseSlideSteps()) + "' step='0.001' min='-5000' max='5000' id='mw_slide'> " + (getSlideStepper()->getPhysicalValueUnit()) + "      <br>      <p style='display: inline-block;'>Photo count:</p>      <input value='" + String(getTimelapsePictureCount()) + "' min='0' max='5000' id='mw_count'> #      <br>      <p style='display: inline-block;'>Exposure Time:</p>      <input value='" + String(getIOBoard()->getCameraExposureTime()) + "' step='0.1' min='0' max='500' id='mw_photo'> s      <br>      <p style='display: inline-block;'>Interval Time:</p>       <input value='" + String(getTimelapseInterval()) + "' min='0' max='500' id='mw_interval'> s      <br>      <p style='display: inline-block;'>Hemisphere: <a href='#' onclick='makeRequest(\"timelapse_set_hemisphere=" + (!isTimelapseInNortherHemisphere() ? 1 : 0) + "\");'><i>" + (isTimelapseInNortherHemisphere() ? "Northern" : "South") + "</i></a></p>       <br>      <button class='block' onclick='makeRequest(\"timelapse_slide=\" + document.getElementById(\"mw_slide\").value + \"&timelapse_count=\" + document.getElementById(\"mw_count\").value + \"&timelapse_photo=\" + document.getElementById(\"mw_photo\").value + \"&timelapse_interval=\" + document.getElementById(\"mw_interval\").value + \"&timelapse_milkyway_settings=1\");'>Shoot Milkyway</button>      <br>      <br>      <p style='display: inline-block;'>Pan set to: " + String(getTimelapsePanSteps()) + "" + (getPanStepper()->getPhysicalValueUnit()) + "</p>      <br>    </div>    <br><br>    <div class='section'>      <h2>Start / Stop</h2>      <br>      <button class='block' onclick='makeRequest(\"timelapse_start\");'>Start Timelapse</button>      <br>      <button class='block' onclick='makeRequest(\"timelapse_stop\");'>Stop Timelapse</button>      <br>      <br>      <button class='block' onclick='makeRequest(\"timelapse_preview_start\");'>Start Preview</button>      <br>      <button class='block' onclick='makeRequest(\"timelapse_preview_stop\");'>Stop Preview</button>      <br>    </div>    <br><br>    <div class='section'>      <h2>Manual</h2>      <br>      <input value='" + String(getTimelapsePanSteps()) + "' type='number' step='0.001' min='-5000' max='5000' id='pan'> " + (getPanStepper()->getPhysicalValueUnit()) + "      <button class='set_button' onclick='makeRequest(\"timelapse_pan=\" + document.getElementById(\"pan\").value);'>Set Pan</button>      <br>      <input value='" + String(getTimelapseSlideSteps()) + "' type='number' step='0.001' min='-5000' max='5000' id='slide'> " + (getSlideStepper()->getPhysicalValueUnit()) + "      <button class='set_button' onclick='makeRequest(\"timelapse_slide=\" + document.getElementById(\"slide\").value);'>Set Slide</button>      <br>      <input value='" + String(getTimelapsePictureCount()) + "' min='0' max='5000' id='count'> #      <button class='set_button' onclick='makeRequest(\"timelapse_count=\" + document.getElementById(\"count\").value);'>Set Picture count</button>      <br>      <input value='" + String(getIOBoard()->getCameraExposureTime()) + "' step='0.1' min='0' max='500' id='photo'> s      <button class='set_button' onclick='makeRequest(\"timelapse_photo=\" + document.getElementById(\"photo\").value);'>Set Photo Interval</button>      <br>      <input value='" + String(getTimelapseInterval()) + "' min='0' max='500' id='interval'> s      <button class='set_button' onclick='makeRequest(\"timelapse_interval=\" + document.getElementById(\"interval\").value);'>Set Timelapse interval</button>      <br>      <br>      <button class='block' onclick='makeRequest(\"timelapse_slide=\" + document.getElementById(\"slide\").value + \"&timelapse_count=\" + document.getElementById(\"count\").value + \"&timelapse_photo=\" + document.getElementById(\"photo\").value + \"&timelapse_interval=\" + document.getElementById(\"interval\").value + \"&timelapse_pan=\" + document.getElementById(\"pan\").value);'>Set all</button>      <br>    </div>    <br><br>    <div class='section'>      <h2>Timelapse-Info</h2>      <br>      <p>Total time: <i>" + secondsToString(getTimelapseTotalTime()) + "</i></p>      <p>End time: <i>" + secondsToTimeString(getTimelapseEndTime()) + "</i></p>      <br>      <p>Total slide: <i>" + getTimelapseTotalSlideDistance() + "</i> " + (getSlideStepper()->getPhysicalValueUnit()) + "</p>      <p>Total pan: <i>" + getTimelapseTotalPanDistance() + "</i> " + (getPanStepper()->getPhysicalValueUnit()) + "</p>      <br>      <p>Video length (24fps): <i>" + secondsToString(getTimelapseVideoTimePrediction(24)) + "</i></p>      <p>Video length (25fps): <i>" + secondsToString(getTimelapseVideoTimePrediction(25)) + "</i></p>      <p>Video length (30fps): <i>" + secondsToString(getTimelapseVideoTimePrediction(30)) + "</i></p>      <p>Video length (60fps): <i>" + secondsToString(getTimelapseVideoTimePrediction(60)) + "</i></p>      <br>    </div>  </div>  <br>  <br>  <a style='text-align: center; display: block;' href='about'>About</a>  <br>  <br></body></html>";
